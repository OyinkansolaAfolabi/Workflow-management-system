...
function save_project(){
  ...
  // $this->db should be your database connection object
  if(!$this->db){
    return 0; // Return an error code or handle the lack of a database connection appropriately
  }
  ...
  if(empty($id)){
    $save = $this->db->query("INSERT INTO project_list SET ".implode(',', $data));
    $project_id = $this->db->insert_id; // Get the inserted project ID
  }else{
    $save = $this->db->query("UPDATE project_list SET ".implode(',', $data)." WHERE id = $id");
    $project_id = $id; // Use the existing project ID
  }

  // Check if the database operation was successful

   if (isset($user_ids) && !empty($user_ids)) {  // Check if set and not empty
      // $this->sendProjectAssignmentEmail($project_id, $user_ids);
      return 1;
    } else {
    return 0;
  }
