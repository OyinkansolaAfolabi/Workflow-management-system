			...
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					if($_SESSION['login_type'] == 2){
						...
						$where = " where concat('[',REPLACE(t.user_ids,',','],['),']') LIKE '%[{$_SESSION['login_id']}]%' ";
					}
							...
							<button type="button" class="btn btn-default btn-sm btn-flat border-info wave-effect text-info dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
		                      Action
			                    ...
			                    <div class="dropdown-menu" style="">
                                  ...
                                  <a class="dropdown-item view_task" data-id = '<?php echo $row['id'] ?>' data-pid = '<?php echo $row['pid'] ?>'  data-task = '<?php echo ucwords($row['task']) ?>'  href="javascript:void(0)">View</a>

                              <div class="dropdown-divider"></div>
                              
                              <a class="dropdown-item edit_task" data-id='<?php echo $row['id'] ?>' data-pid='<?php echo $row['pid'] ?>' data-task='<?php echo ucwords($row['task']) ?>' href="javascript:void(0)">Edit</a>

                              <div class="dropdown-divider"></div>
                              <?php if($_SESSION['login_type'] != 3): ?>
                              <a class="dropdown-item delete_task"data-id = '<?php echo $row['id'] ?>' data-pid = '<?php echo $row['pid'] ?>'  data-task = '<?php echo ucwords($row['task']) ?>'  href="javascript:void(0)">Delete</a>
                          <?php endif; ?>
								</div>
						</td>
					</tr>	
...
<script>
	...
	$('.new_productivity').click(function(){
		uni_modal("<i class='fa fa-plus'></i> New Progress for: "+$(this).attr('data-task'),"manage_progress.php?id="+$(this).attr('data-id')+"&pid="+$(this).attr('data-pid'),'large')
	})


    $('.view_task').click(function(){
        uni_modal("<i class='fa fa-plus'></i> Task Details","view_task.php?id="+$(this).attr('data-id')+"&pid="+$(this).attr('data-pid'),"mid-large")
    })
    
    $('.edit_task').click(function(){
    uni_modal("<i class='fa fa-plus'></i> Edit Task: " + $(this).attr('data-task'), "manage_task.php?id=" + $(this).attr('data-id') + "&pid=" + $(this).attr('data-pid'), "mid-large");
});
